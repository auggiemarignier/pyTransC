...
def rescale_gmm_covariances(gmm: GaussianMixture,
    ...
    if cov_type == 'full':
        ...
        s_squared = s ** 2
        new_covariances = gmm.covariances_ * s_squared
        gmm.covariances_ = new_covariances
        if verbose:
        ...
        raise NotImplementedError(f"Unsupported covariance_type: '{cov_type}'")
...
def build_auto_pseudo(ensemble_per_state=None, standardize=True, **kwargs):
    """Build automatic pseudo prior using standardized input ensembles"""
    gmms = []
    for state, ens in enumerate(ensemble_per_state):
        if(standardize):
            ...
            gmm = GaussianMixture(**kwargs).fit(ens)
        gmms.append(gmm)

    def _log_pseudo_prior(x, state, gmms):
        if (x.ndim == 1):
            x = x.reshape(1, -1)
        return gmms[state].score_samples(x)

    log_pseudo_prior = partial(_log_pseudo_prior, gmms=gmms)
